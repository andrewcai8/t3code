...
read its limits directly. At each check, T3 Code sends it a one-word message on Haiku and reads the
session and weekly windows from the reply. That spends a sliver of the account's usage. Model-specific
