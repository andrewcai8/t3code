 ...
 *
 * An awake cloud box counts once against the account its chat runs on. The
 * lease records the primary account routing froze into the box's manifest,
 * not the account the request hinted at. A local thread counts while a turn
 * is running on its provider instance. A source that cannot be read counts
 * as zero, so routing degrades to usage alone instead of refusing.
 */
  ...
  Effect.all(
    {
      boxes: Effect.tryPromise(async () =>
        (await leases.awake()).map((lease) => ProviderInstanceId.make(lease.providerInstanceId)),
      ).pipe(
        Effect.catch((cause) =>
